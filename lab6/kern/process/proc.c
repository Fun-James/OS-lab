...
alloc_proc(void)
{
    ...
    if (proc != NULL)
    {
        ...
        // LAB6: 2313447 (update LAB5 steps)
        /*
...
int do_fork(uint32_t clone_flags, uintptr_t stack, struct trapframe *tf)
{
    ...
    // LAB5 更新：设置父进程为当前进程，并确保父进程的 wait_state 为 0
    proc->parent = current;
    assert(current->wait_state == 0);  // 确保父进程没有在等待状态
    ...
    // 5. LAB5 更新：使用 set_links 将 proc_struct 插入到 hash_list 和 proc_list 中，并设置进程关系
    bool intr_flag;
    local_intr_save(intr_flag);  // 禁用中断，保证原子操作
    {
        proc->pid = get_pid();    // 分配唯一的 PID
        hash_proc(proc);          // 加入哈希表
        set_links(proc);          // LAB5: 使用 set_links 设置进程关系链接（包括加入进程链表和更新进程数量）
    }

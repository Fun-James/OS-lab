...
当前的代码是线性查找，复杂度为 O(n) ，改进可使用大小分类的多个链表，优化查询效率。

* 分离空闲链表

当前仅合并相邻块，改进可添加延迟合并或定期压缩碎片，移动已分配块来合并大块空闲区域。
...
在传统的 x86 系统环境下，操作系统可以通过 BIOS 中断调用来获取内存信息：

```c++
struct memory_map_entry {
    uint64_t base_addr;
    uint64_t length;
    uint32_t type;  // 1表示可用内存, 其他表示保留区域
    uint32_t extended_attrs;
};

// 通过 BIOS 调用获取详细的内存映射表
void detect_memory_via_bios() {
...
引导加载程序将收集到的所有内存段信息整理成一个列表，并将其存放在内存的某个约定好的位置。然后，它会将这个列表的地址和条目数量作为启动参数之一传递给操作系统内核。内核启动后的早期阶段，首要任务之一就是解析这个内存布局图，从而了解哪些物理地址范围是可以使用的。

## 2. UEFI 运行时服务

1. 代码示例

对于 UEFI 系统，可以通过 UEFI 运行时服务获取内存映射：

```c++
EFI_STATUS get_uefi_memory_map(EFI_MEMORY_DESCRIPTOR **memory_map, 
                              UINTN *memory_map_size, 
                              UINTN *descriptor_size) {
    // UEFI 提供标准接口比如可用内存、固件保留等，获取完整内存映射
}
```

* 工作原理

与 BIOS 类似，引导加载程序获取这张内存地图后，会将其传递给操作系统内核。在 OS 完全接管硬件控制权之前，它必须调用另一个 UEFI 服务 `ExitBootServices()`。一旦这个函数被调用，所有的 UEFI 启动服务将永久失效，OS 将独立运行。所以，获取内存地图必须在调用 `ExitBootServices()` 之前完成。
...
// 解析引导程序传递的内存映射信息
void parse_multiboot_memory_map(struct multiboot_info *mb_info) {
...
引导程序负责执行平台相关的操作来探测内存，它将从固件获取的、格式可能不同的信息，转换成一个标准化的数据结构。在跳转到内核入口点之前，引导程序会将指向这个标准化数据结构的指针存放在一个特定的寄存器中给内核。
